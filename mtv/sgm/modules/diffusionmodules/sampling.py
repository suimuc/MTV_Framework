...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None,
                audio_emb_vocal=None, audio_emb_vocal_1=None, audio_emb_accm=None, audio_emb_music = None, latent_frame_mask = None):
        additional_model_inputs = {"audio_emb_vocal": audio_emb_vocal,
                                    "audio_emb_vocal_1": audio_emb_vocal_1,
                                    "latent_frame_mask": latent_frame_mask,
                                   "audio_emb_accm": audio_emb_accm,
        ...
        if isinstance(scale, torch.Tensor) == False and scale == 1:
            ...
            additional_model_inputs["timesteps_t0"] = x.new_ones([x.shape[0]]) * 0
            ...
            additional_model_inputs["timesteps_t0"] = torch.cat([x.new_ones([x.shape[0]]) * 0] * 2)
            uc_audio_emb = torch.zeros_like(audio_emb_vocal)
            additional_model_inputs["audio_emb_vocal"] = torch.cat([uc_audio_emb, audio_emb_vocal], dim=0)
            if (audio_emb_vocal_1 is not None):
                additional_model_inputs["audio_emb_vocal_1"] = torch.cat([uc_audio_emb, audio_emb_vocal_1], dim=0)
            additional_model_inputs["audio_emb_accm"] = torch.cat([uc_audio_emb, audio_emb_accm], dim=0)
            additional_model_inputs["audio_emb_music"] = torch.cat([uc_audio_emb, audio_emb_music], dim=0)
...
class VPSDEDPMPP2MSampler(VideoDDIMSampler):
    ...
    def sampler_step(
        ...
        scale=None,
        scale_emb=None,
        audio_emb_vocal=None,
        audio_emb_vocal_1=None,
        audio_emb_accm=None,
        audio_emb_music=None,
        latent_frame_mask = None,
    ):
        # import pdb; pdb.set_trace()
        denoised = self.denoise(
            x, denoiser, alpha_cumprod_sqrt, cond, uc, timestep, idx, scale=scale, scale_emb=scale_emb,
            audio_emb_vocal=audio_emb_vocal, audio_emb_vocal_1=audio_emb_vocal_1, audio_emb_accm=audio_emb_accm, audio_emb_music=audio_emb_music, latent_frame_mask = latent_frame_mask
        ).to(torch.float32)
        if idx == 1:
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, scale=None, scale_emb=None,
        ...
        if (latent_frame_mask is not None):
            self.fixed_frames = (~latent_frame_mask).sum().item()
        else:
            self.fixed_frames = 0

        if self.fixed_frames > 0:
            prefix_frames = x[:, : self.fixed_frames].clone()
        old_denoised = None
        for i in self.get_sigma_gen(num_sigmas):
            if self.fixed_frames > 0:
                if self.sdedit:
            ...
            x, old_denoised = self.sampler_step(
                ...
                scale=scale,
                scale_emb=scale_emb,
                audio_emb_vocal=audio_emb_vocal,
                audio_emb_vocal_1=audio_emb_vocal_1,
                audio_emb_accm=audio_emb_accm,
                audio_emb_music=audio_emb_music,
                latent_frame_mask = latent_frame_mask,
