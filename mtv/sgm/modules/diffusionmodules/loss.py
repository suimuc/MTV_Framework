...
class VideoDiffusionLoss(StandardDiffusionLoss):
    ...
    def __call__(self, network, denoiser, conditioner, input, batch):
        ...
        if ("latent_frame_mask" in batch):
            alphas_cumprod_sqrt_t0, idx_t0 = self.sigma_sampler(input.shape[0], rand = torch.tensor([0]), return_idx=True)
            alphas_cumprod_sqrt_t0 = alphas_cumprod_sqrt_t0.to(input.device)
            idx_t0 = idx_t0.to(input.device)
        ...
        if ("latent_frame_mask" in batch):
            torch.distributed.broadcast(idx_t0, src=src, group=mpu.get_model_parallel_group())
            torch.distributed.broadcast(alphas_cumprod_sqrt_t0, src=src, group=mpu.get_model_parallel_group())

        additional_model_inputs["idx"] = idx

        if ("latent_frame_mask" in batch):
            additional_model_inputs["idx_t0"] = idx_t0
            additional_model_inputs["latent_frame_mask"] = batch["latent_frame_mask"]
        ...
        if ("latent_frame_mask" in batch):
            latent_frame_mask = batch["latent_frame_mask"]
            noised_t0 = input.float() * append_dims(alphas_cumprod_sqrt_t0, input.ndim) + noise * append_dims(
                (1 - alphas_cumprod_sqrt_t0**2) ** 0.5, input.ndim
            )
            noised_input = torch.where(latent_frame_mask[:, :, None, None, None], noised_input, noised_t0)

        if "concat_images" in batch.keys():
            cond["concat"] = batch["concat_images"]

        if ("latent_frame_mask" in batch):
            model_output = denoiser(network, noised_input, alphas_cumprod_sqrt, cond, sigma_t0 = alphas_cumprod_sqrt_t0, **additional_model_inputs)
        else:
