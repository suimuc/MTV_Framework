...
def attention_fn_default(query_layer, key_layer, value_layer, attention_mask,
    ...
    value_layer = value_layer.unsqueeze(2).expand(-1, -1, num_query_heads // num_kv_heads, -1, -1).contiguous().view(
        batch_size, num_query_heads, *value_layer.shape[2:])
...
def cross_attention_forward_default(self, hidden_states, cross_attention_mask, encoder_outputs, **kw_args):
    is_audio = kw_args.get("is_audio", False)
    is_audio_1 = kw_args.get("is_audio_1", False)
    is_audio_effect = kw_args.get("is_audio_effect", False)



    if is_audio:
        self = self.transformer.layers[kw_args['layer_id']].audio_attn
    elif is_audio_1:
        self = self.transformer.layers[kw_args['layer_id']].audio_attn_second
    elif is_audio_effect:
        self = self.transformer.layers[kw_args['layer_id']].audio_attn_effect
    else:
        raise ValueError("one of is_audio and is_audio_1 and is_audio_effect should be true")
    ...
    context_layer = attention_fn(query_layer, key_layer, value_layer, cross_attention_mask, dropout_fn,
                                 cross_attention=True, mem_cross=mem_cross, **kw_args)
    context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
    new_context_layer_shape = context_layer.size()[:-2] + (self.hidden_size_per_partition,)
    # [b, s, hp]
    context_layer = context_layer.view(*new_context_layer_shape)
...
def routing_forward_default(self, hidden_states, **kw_args):
    ...
    router_logits = torch.randn((batch_size * sequence_length, num_experts), device=hidden_states.device,
    ...
    if self.transformer.num_experts == 1 or expert_id > -1:
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_state = hidden_states[
                top_x_list]  # I don't know why using hidden_states[None, top_x_list].reshape(-1, hidden_dim)
            current_hidden_states = mlp_forward(current_state, expert_id=expert_idx, **kw_args) * fwd_weight[
...
def layer_forward_default(self, hidden_states, mask, *args, **kw_args):
    ...
    if self.training and self.drop_path > 0.:
        ...
        random_tensor = (1 - self.drop_path
                         + torch.rand((attention_output.shape[0],), dtype=attention_output.dtype,
                                      device=attention_output.device)).floor_() / (1 - self.drop_path)
    ...
    if self.training and self.drop_path > 0.:
        random_tensor = (1 - self.drop_path
                         + torch.rand((mlp_output.shape[0],), dtype=mlp_output.dtype,
                                      device=mlp_output.device)).floor_() / (1 - self.drop_path)
