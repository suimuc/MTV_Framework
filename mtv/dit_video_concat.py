...
from sgm.models.base_model import BaseModel, BaseMixin, non_conflict
...
class ImagePatchEmbeddingMixin(BaseMixin):
    ...
    def word_embedding_forward(self, input_ids, **kwargs):
        ...
        B, T = images.shape[:2]
        emb = images.view(-1, *images.shape[2:])  # ((b t), in_channels, h, w)
...
class Rotary3DPositionEmbeddingMixin(BaseMixin):
    def __init__(
        ...
        if learnable_pos_embed:
            num_patches = height * width * compressed_num_frames + text_length  # 30*45*13+226=17776
    ...
    def position_embedding_forward(self, position_ids, **kwargs):
        if self.pos_embedding is not None:
            pos_emb_origin = self.pos_embedding[:, :self.text_length + kwargs["seq_length"]]
            return pos_emb_origin
        else:
            return None
    ...
    def attention_fn(
        ...
        is_cross = kwargs.get("is_cross", False)

        # if not is_cross:
        #     query_layer[:, :, self.text_length:-ref_length] = self.rotary(query_layer[:, :, self.text_length:-ref_length])
        #     key_layer[:, :, self.text_length:-ref_length] = self.rotary(key_layer[:, :, self.text_length:-ref_length])
        #     if self.rot_v:
        #         value_layer[:, :, self.text_length:-ref_length] = self.rotary(value_layer[:, :, self.text_length:-ref_length])
        # else:
        #     query_layer = self.rotary(query_layer)
...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        # import pdb; pdb.set_trace()
        img_attention_input_t = modulate(img_attention_input, shift_msa, scale_msa)
        if ("latent_frame_mask" in kwargs and latent_frame_mask is not None):
            img_attention_input_t0 = modulate(img_attention_input, shift_msa_t0, scale_msa_t0)
            img_attention_input = self.t_mask_select(latent_frame_mask, img_attention_input_t, img_attention_input_t0, 13, 1350)
            # print("OK")
        else:
            img_attention_input = img_attention_input_t
        ...
        img_attention_output_t = gate_msa * img_attention_output
        if ("latent_frame_mask" in kwargs and latent_frame_mask is not None):
            img_attention_output_t0 = gate_msa_t0 * img_attention_output
            img_attention_output = self.t_mask_select(latent_frame_mask, img_attention_output_t, img_attention_output_t0, 13, 1350)
        else:
            img_attention_output = img_attention_output_t

        # img_hidden_states = img_hidden_states + gate_msa * img_attention_output  # (b,(t n),d)
        img_hidden_states = img_hidden_states + img_attention_output
        text_hidden_states = text_hidden_states + text_gate_msa * text_attention_output  # (b,n,d)




        if layer.add_audio_module:
            # import pdb; pdb.set_trace()
            if (self.is_multi_person):
                audio_vocal, audio_vocal_1, audio_accm, audio_music = torch.chunk(audio, 4, dim=1)
            else:
                audio_vocal, audio_accm, audio_music = torch.chunk(audio, 3, dim=1)

            audio_attn_input = layer.audio_input_layernorm(img_hidden_states)  # vision (b,(t n),d)
            audio_attn_input = rearrange(audio_attn_input, "b (t n) d -> (b t) n d", t=video_length)
            kwargs['encoder_outputs'] = audio_vocal
            kwargs['is_cross'] = True
            kwargs['is_audio_1'] = False
            kwargs['is_audio'] = True
            kwargs['is_audio_effect'] = False
            audio_attn_output = layer.audio_attn(audio_attn_input, mask, **kwargs)
            audio_attn_output = rearrange(audio_attn_output, "(b t) n d -> b (t n) d", t=video_length)
            if (self.is_multi_person):
                audio_attn_input_1 = layer.audio_input_layernorm_second(img_hidden_states)  # vision (b,(t n),d)
                audio_attn_input_1 = rearrange(audio_attn_input_1, "b (t n) d -> (b t) n d", t=video_length)
                kwargs['encoder_outputs'] = audio_vocal_1
                kwargs['is_cross'] = True
                kwargs['is_audio_1'] = True
                kwargs['is_audio'] = False
                kwargs['is_audio_effect'] = False
                audio_attn_output_1 = layer.audio_attn_second(audio_attn_input_1, mask, **kwargs)
                audio_attn_output_1 = rearrange(audio_attn_output_1, "(b t) n d -> b (t n) d", t=video_length)

            audio_attn_input_effect = layer.audio_input_layernorm_effect(img_hidden_states)  # vision (b,(t n),d)
            audio_attn_input_effect = rearrange(audio_attn_input_effect, "b (t n) d -> (b t) n d", t=video_length)
            kwargs['encoder_outputs'] = audio_accm
            kwargs['is_cross'] = True
            kwargs['is_audio_1'] = False
            kwargs['is_audio'] = False
            kwargs['is_audio_effect'] = True
            audio_attn_output_effect = layer.audio_attn_effect(audio_attn_input_effect, mask, **kwargs)
            audio_attn_output_effect = rearrange(audio_attn_output_effect, "(b t) n d -> b (t n) d", t=video_length)

            if (self.is_multi_person):
                img_hidden_states = img_hidden_states + audio_attn_output + audio_attn_output_1 + audio_attn_output_effect
            else:
                img_hidden_states = img_hidden_states + audio_attn_output + audio_attn_output_effect

            audio_music = rearrange(audio_music, "(b t) n d -> b (t n) d", t = video_length)
            audio_music = audio_music.mean(dim = 1)
            music_scale, music_shift = layer.audio_music_adaLn(audio_music).unsqueeze(1).chunk(2, dim = 2)
            img_hidden_states = img_hidden_states * (1 + music_scale) + music_shift

            kwargs['is_cross'] = False
            kwargs['is_audio'] = False
            kwargs['is_audio_1'] = False
            kwargs['is_audio_effect'] = False
        ...
        img_mlp_input_t = modulate(img_mlp_input, shift_mlp, scale_mlp)
        if ("latent_frame_mask" in kwargs and latent_frame_mask is not None):
            img_mlp_input_t0 = modulate(img_mlp_input, shift_mlp_t0, scale_mlp_t0)
            img_mlp_input = self.t_mask_select(latent_frame_mask, img_mlp_input_t, img_mlp_input_t0, 13, 1350)
        else:
            img_mlp_input = img_mlp_input_t
        ...
        img_mlp_output_t = gate_mlp * img_mlp_output

        # import pdb; pdb.set_trace()
        if ("latent_frame_mask" in kwargs and latent_frame_mask is not None):
            img_mlp_output_t0 = gate_mlp_t0 * img_mlp_output
            img_mlp_output = self.t_mask_select(latent_frame_mask, img_mlp_output_t, img_mlp_output_t0, 13, 1350)
        else:
            img_mlp_output = img_mlp_output_t

        img_hidden_states = img_hidden_states + img_mlp_output  # vision (b,(t n),d)
...
class DiffusionTransformer(BaseModel):
    def __init__(
        ...
        transformer_args.add_audio_module = add_audio_module
        self.is_multi_person = transformer_args.is_multi_person
        kwargs["is_multi_person"] = transformer_args.is_multi_person
        ...
        if self.input_time == "adaln":
            ...
            self.add_mixin(
                ...
                instantiate_from_config(
                    ...
                    is_multi_person=self.is_multi_person,
                ),
            )
        else:
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if ("timesteps_t0" in kwargs):
            
            t0_emb = timestep_embedding(kwargs["timesteps_t0"], self.model_channels, repeat_only=False, dtype=self.dtype)
            emb_t0 = self.time_embed(t0_emb)
            kwargs["emb_t0"] = emb_t0
        ...
        kwargs["video_length"] = t

        if self.add_audio_module:
            kwargs["audio_emb_vocal"] = kwargs["audio_emb_vocal"].to(self.dtype)
            if (self.is_multi_person):
                kwargs["audio_emb_vocal_1"] = kwargs["audio_emb_vocal_1"].to(self.dtype)
            kwargs["audio_emb_accm"] = kwargs["audio_emb_accm"].to(self.dtype)
            kwargs["audio_emb_music"] = kwargs["audio_emb_music"].to(self.dtype)
        else:
            kwargs["audio_emb_vocal"] = None
            kwargs["audio_emb_vocal_1"] = None
            kwargs["audio_emb_accm"] = None
            kwargs["audio_emb_music"] = None
